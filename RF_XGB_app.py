...
def preprocess_and_load_data():
    ...
    if os.path.exists(processed_file):
        ...
        if not os.path.exists(raw_file):
            st.error(f"데이터 파일을 찾을 수 없습니다: {raw_file}")
            st.stop()
...
# 2. 모델 학습 및 예측 공통 함수
# ------------------------------------------------
def train_and_predict(model_type, X_train, y_train, X_test, X_columns):
    X_tr = X_train.astype(float) if model_type == 'XGBoost' else X_train
    X_te = X_test.astype(float) if model_type == 'XGBoost' else X_test
    
    y_train_class = (y_train > 0).astype(int)
    mask_positive = y_train > 0

    if model_type == 'Random Forest':
        clf = RandomForestClassifier(n_estimators=500, max_depth=10, min_samples_split=3, min_samples_leaf=2, random_state=42, n_jobs=-1)
        reg = RandomForestRegressor(n_estimators=500, max_depth=10, min_samples_split=3, min_samples_leaf=2, random_state=42, n_jobs=-1)
    else: # XGBoost
        clf = XGBClassifier(n_estimators=500, max_depth=6, learning_rate=0.05, random_state=42, n_jobs=-1, eval_metric='logloss')
        reg = XGBRegressor(n_estimators=500, max_depth=6, learning_rate=0.05, random_state=42, n_jobs=-1)

    clf.fit(X_tr, y_train_class)
    prob_order = clf.predict_proba(X_te)[:, 1]

    reg.fit(X_tr[mask_positive], np.log1p(y_train[mask_positive]))
    pred_qty = np.expm1(reg.predict(X_te))
    ...
    return clf, reg, final_predictions

def generate_future_predictions(model_type, clf, reg, prod_full, future_weeks, selected_product, X_columns, buffer_rate):
    ...
    for f_date in future_dates:
        ...
        df_f_encoded = df_f_encoded.reindex(columns=X_columns, fill_value=0)
        
        if model_type == 'XGBoost':
            df_f_encoded = df_f_encoded.astype(float)
            
        p_prob = clf.predict_proba(df_f_encoded)[:, 1][0]
        if p_prob >= 0.3:
            p_val = max(0, np.expm1(reg.predict(df_f_encoded)[0]))
        else:
            p_val = 0.0
...
# 3. 대시보드 UI 구성
# ------------------------------------------------
st.set_page_config(page_title="통합 주간 주문 예측 시스템", layout="wide")
st.title("🤖 AI 주간 주문 예측 및 생산 계획 시스템")
st.write("Random Forest 및 XGBoost 모델을 선택하거나 두 모델의 예측 결과를 동시에 비교할 수 있습니다.")

weekly_df, X, y, unique_products = preprocess_and_load_data()

# 사이드바 설정
st.sidebar.header("⚙️ 예측 및 모델 설정")
model_choice = st.sidebar.radio("사용 알고리즘 선택", ["Random Forest", "XGBoost", "두 모델 비교 분석"])
...
if model_choice in ["Random Forest", "XGBoost"]:
    with st.spinner(f'{model_choice} 모델 학습 및 분석 진행 중...'):
        clf, reg, final_predictions = train_and_predict(model_choice, X_train, y_train, X_test, X.columns)
    ...
    st.subheader(f"📊 [{model_choice}] 전체 공장 통합 예측 정확도")
    col_g1, col_g2 = st.columns(2)
    col_g1.metric("전체 평균 주간 오차율 (WAPE)", f"{global_wape:,.2f} %")
    col_g2.metric("전체 평균 예측 정확도", f"{global_accuracy:,.2f} %")

    st.divider()
    st.subheader(f"🎯 선택 제품 [{selected_product}] 오차율 분석 ({model_choice})")
    ...
    if prod_test.empty:
        ...
        ax.plot(prod_test['order_date'], prod_test['예측값'], label=f'{model_choice} 예측량', color='darkorange', linestyle='--', linewidth=2, marker='^', markersize=8)
    ...
    # 미래 예측
    st.divider()
    st.subheader(f"🚀 [{selected_product}] 향후 {future_weeks}주간 예측 및 필요 생산 계획 ({model_choice})")
    
    prod_full = weekly_df[weekly_df['제품코드'] == selected_product].sort_values('order_date').copy()
    
    if not prod_full.empty:
        future_df = generate_future_predictions(model_choice, clf, reg, prod_full, future_weeks, selected_product, X.columns, buffer_rate)
        ...
        with tab1:
            ...
            ax2.plot(last_hist_x, last_hist_y, label=f'향후 {future_weeks}주 미래 예측선', color='#dc2626', linestyle='--', linewidth=2.5, marker='o', markersize=8)
            ax2.set_title(f"[{selected_product}] 미래 {future_weeks}주 주문량 예측 트렌드 ({model_choice})", fontsize=15, pad=12)
        ...
        <div style="background-color: #eff6ff; padding: 24px; border-radius: 12px; border: 2px solid #2563eb; margin-top: 20px;">
            <div style="font-size: 20px; font-weight: bold; color: #1e40af; margin-bottom: 8px;">
                💡 [최종 결론] 향후 {future_weeks}주간 총 필요 생산 계획 수량 ({model_choice})
            </div>
...
# ------------------------------------------------
    ...
    with st.spinner('Random Forest 및 XGBoost 모델 학습 및 비교 중...'):
        clf_rf, reg_rf, pred_rf = train_and_predict('Random Forest', X_train, y_train, X_test, X.columns)
        clf_xgb, reg_xgb, pred_xgb = train_and_predict('XGBoost', X_train, y_train, X_test, X.columns)

    wape_rf = calculate_wape(y_test, pred_rf)
    wape_xgb = calculate_wape(y_test, pred_xgb)

    st.subheader("⚖️ 모델별 통합 오차율 비교 (WAPE)")
    col_c1, col_c2 = st.columns(2)
    col_c1.metric("Random Forest 평균 WAPE", f"{wape_rf:,.2f} %")
    col_c2.metric("XGBoost 평균 WAPE", f"{wape_xgb:,.2f} %", delta=f"{wape_rf - wape_xgb:,.2f}%p 개선" if wape_xgb < wape_rf else f"{wape_xgb - wape_rf:,.2f}%p 차이")

    st.divider()
    st.subheader(f"🎯 [{selected_product}] 제품 두 모델 예측 비교")

    prod_test = test_df[test_df['제품코드'] == selected_product].copy()
    if not prod_test.empty:
        prod_test['RF_예측'] = pred_rf[test_df['제품코드'] == selected_product]
        prod_test['XGB_예측'] = pred_xgb[test_df['제품코드'] == selected_product]

        prod_wape_rf = calculate_wape(prod_test['주간주문량'], prod_test['RF_예측'])
        prod_wape_xgb = calculate_wape(prod_test['주간주문량'], prod_test['XGB_예측'])

        fig_comp, ax_comp = plt.subplots(figsize=(15, 6))
        ax_comp.plot(prod_test['order_date'], prod_test['주간주문량'], label='실제 주문량', color='green', alpha=0.8, marker='s')
        ax_comp.plot(prod_test['order_date'], prod_test['RF_예측'], label=f'RF 예측 (WAPE: {prod_wape_rf:.2f}%)', color='blue', linestyle='--', marker='o')
        ax_comp.plot(prod_test['order_date'], prod_test['XGB_예측'], label=f'XGBoost 예측 (WAPE: {prod_wape_xgb:.2f}%)', color='red', linestyle='--', marker='^')

        ax_comp.set_title(f"[{selected_product}] 모델별 예측 비교 차트", fontsize=15)
        ax_comp.set_xlabel("날짜")
        ax_comp.set_ylabel("주간 주문량")
        ax_comp.legend()
        plt.xticks(rotation=45)
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        st.pyplot(fig_comp)
